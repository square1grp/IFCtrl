...
class IntelliFluxQueryManager(object):
                ...
                if cache_data_detail_df.count().widget_name == 1:
                    ...
                    if time_stamp_from is not None and time_stamp_to is not None and cache_data_detail_df.last_modification_time.item() > cache_expiry_date:
                        if time_stamp_from >= cache_data_detail_df.time_stamp_from.item()  and time_stamp_to <= cache_data_detail_df.time_stamp_to.item():
                            ...
                            modified_query = replace_date_filters(sql_query,time_stamp_from,cache_data_detail_df.time_stamp_to.item())
                            ...
                            widget_data =  psql.sqldf(panda_Sql_query, locals())
                            self.updateMasterDataFrame(master_data_frame,widget_name,user_id,database_id,cache_data_detail_df.time_stamp_from.item(),time_stamp_to)
                        else:
                            print("No matched")
                    else:
                         json_widget_data = getWidgetData(database_id,sql_query)
    ...
    def updateMasterDataFrame(self,dataFrameToUpdate,widget_name,user_id,database_id,time_stamp_from,time_stamp_to):
        ...
        try:
            current_date_time = datetime.now()
            dataFrameToUpdate.loc[(master_data_frame['widget_name'] == widget_name) & (master_data_frame['user_id'] == user_id) & (master_data_frame['database_id'] == database_id), 'time_stamp_from'] = time_stamp_from
            dataFrameToUpdate.loc[(master_data_frame['widget_name'] == widget_name) & (master_data_frame['user_id'] == user_id) & (master_data_frame['database_id'] == database_id), 'time_stamp_to'] = time_stamp_to
            dataFrameToUpdate.loc[(master_data_frame['widget_name'] == widget_name) & (master_data_frame['user_id'] == user_id) & (master_data_frame['database_id'] == database_id), 'last_modification_time'] = current_date_time
