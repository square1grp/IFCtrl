...
def get_layout():
    nav_items = current_user.get_page_nav_items()
...
def display_page(pathname, n_clicks, time_stamp_from, time_stamp_to, database_id):
    if n_clicks and n_clicks > 0:
        current_user.set_time_stamp_from(time_stamp_from)
        current_user.set_time_stamp_to(time_stamp_to)
        current_user.set_user_database_id(database_id)

    slug = ''
    if pathname is not None:
        slug = pathname.replace('/', '')

    page_items = current_user.get_page_items()
